  ...
  if (matcherType.compare("MAT_BF") == 0) {
    ...
    // with all other binary descriptors
      ...
      throw invalid_argument(descriptorType +
                             " is not a valid descriptorCategory");
    }
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  if (descriptorType.compare("BRISK") == 0) {
    ...
    throw invalid_argument(descriptorType +
                           " descriptor method not supported!");
  }
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  // Detector parameters
  int blockSize =
      2; // for every pixel, a blockSize x blockSize neighborhood is considered
  double maxOverlap = 0.0; // max. permissible overlap between two features in %
  int apertureSize = 3; // aperture parameter for Sobel operator (must be odd)
  int minResponse =
      100; // minimum value fo a corner in the 8bit scaled response matrix
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
  cout << detectorType + " detector with n= " << keypoints.size()
       << "keypoints in " << 1000 * t / 1.0 << " ms " << endl;
}